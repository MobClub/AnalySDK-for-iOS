...
- (void)viewDidLoad
{
    ...
    button = [UIButton buttonWithType:UIButtonTypeSystem];
    [button setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [button setTitle:@"开始事件" forState:UIControlStateNormal];
    [button addTarget:self action:@selector(behaviorStartClick) forControlEvents:UIControlEventTouchUpInside];
    button.frame = CGRectMake(midX - 80, midY, width, height);

    [self.view addSubview:button];
    
    button = [UIButton buttonWithType:UIButtonTypeSystem];
    [button setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [button setTitle:@"结束事件" forState:UIControlStateNormal];
    [button addTarget:self action:@selector(behaviorEndClick) forControlEvents:UIControlEventTouchUpInside];
    button.frame = CGRectMake(midX + 80, midY, width, height);
    [self.view addSubview:button];
}
...
- (void)behaviorStartClick
{
    [AnalySDK behaviorStart:@"eventName_test" eventTag:@"tag01" eventParams:@{@"country":@"China"}];
}

- (void)behaviorEndClick
{
    [AnalySDK behaviorEnd:@"eventName_test" eventTag:@"tag01" eventParams:@{@"country":@"China",@"city":@"shanghai"}];
}
